class Sprite {
    constructor(left, right, up, down, lastPressed, bomberID, height) {
        this.left = left;
        this.right = right;
        this.up = up;
        this.down = down;
    ...
    drawImgUp(){
        if(this.frameCounter < this.totalFrames){
            ctx.drawImage(this.up, this.width*this.ssNum, 0, this.width, this.height, g.playerArr[this.bomberID].x - 22, g.playerArr[this.bomberID].y - 34, this.width*this.scale, this.height*this.scale);
        }
        if(this.frameCounter % this.frameRate == 0){
            this.ssNum++;
        }
    ...
    drawImgDown(){
        if(this.frameCounter < this.totalFrames){
            ctx.drawImage(this.down, this.width*this.ssNum, 0, this.width, this.height, g.playerArr[this.bomberID].x - 22, g.playerArr[this.bomberID].y - 34, this.width*this.scale, this.height*this.scale);
        }
        if(this.frameCounter % this.frameRate == 0){
            this.ssNum++;
        }
